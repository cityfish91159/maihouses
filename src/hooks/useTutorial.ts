...
interface TutorialStep {
  ...
  mood: 'wave' | 'thinking' | 'excited' | 'sleep' | 'idle' | 'happy' | 'celebrate' | 'confused' | 'peek' | 'shy';
  message: string;
  action?: () => void;
}
...
export function useTutorial() {
  ...
  const safeSetMood = useCallback((mood: TutorialStep['mood'], message: string) => {
    try {
      setMood(mood);
      addMessage(message);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const debouncedReset = debounce(resetTimer, 1000, { leading: true, trailing: false });
