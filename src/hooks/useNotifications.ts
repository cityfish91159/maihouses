...
interface RawConversationData {
    ...
    property?: Array<{ public_id: string; title: string; images: string[] }> | null;
    messages?: Array<{
        content: string;
        created_at: string;
...
function transformConversation(conv: RawConversationData, isAgent: boolean): ConversationListItem {
    ...
    if (profileData) {
        counterpartName = profileData.name || profileData.email?.split('@')[0] || (isAgent ? 'User' : 'Agent');
    ...
    const fetchNotifications = useCallback(async () => {
        ...
        try {
            ...
            if (isAgent) {
                ...
                // 計算總未讀數
                const totalUnread = result?.reduce((sum: number, conv: RawConversationData) => sum + (conv.unread_agent || 0), 0) || 0;
                ...
                // 計算總未讀數
                const totalUnread = result?.reduce((sum: number, conv: RawConversationData) => sum + (conv.unread_consumer || 0), 0) || 0;
                setCount(totalUnread);
