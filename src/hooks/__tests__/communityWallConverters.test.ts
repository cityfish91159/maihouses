...
describe("resolveAuthorDisplay", () => {
  it("優先使用作者姓名，保留樓層資訊", () => {
    const result = resolveAuthorDisplay("123456789", {
      name: "王小明",
      role: "resident",
      floor: "12F",
    });
...
describe("convertApiReview", () => {
  ...
  it("房仲有姓名時直接使用", () => {
    const converted = convertApiReview({
      ...baseReview,
      agent: { name: "張房仲" },
    });
...
describe("formatTimeAgo", () => {
  it("handles key time windows", () => {
    ...
    expect(
      formatTimeAgo(new Date(now - 30 * 60 * 1000).toISOString()),
    ).toContain("分鐘前");
    expect(
      formatTimeAgo(new Date(now - 3 * 24 * 60 * 60 * 1000).toISOString()),
    ).toContain("天前");
    expect(
      formatTimeAgo(new Date(now - 2 * 7 * 24 * 60 * 60 * 1000).toISOString()),
    ).toContain("週前");
  });
});
...
describe("convertApiData", () => {
  it("returns neutral placeholders when posts/reviews/questions missing", () => {
    const converted = convertApiData({
      ...
      posts: {
        public: null as unknown as CommunityPost[],
        private: null as unknown as CommunityPost[],
        publicTotal: 0,
        privateTotal: 0,
      },
