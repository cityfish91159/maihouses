...
describe('usePropertyDraft', () => {
  ...
  it('migrates draft from anonymous to user', () => {
    ...
    localStorage.setItem('mh_draft_upload_anonymous', JSON.stringify(stored));
    const { result, rerender } = renderHook(({ uid }) => usePropertyDraft(baseForm, uid), {
  ...
  it('saves and restores trustEnabled state', () => {
    ...
    const { result } = renderHook(() => usePropertyDraft(formWithTrust, 'trust-test'));

    // 等待自動存檔
    act(() => vi.advanceTimersByTime(1000));

    // 驗證 localStorage 有儲存 trustEnabled
    const saved = JSON.parse(localStorage.getItem('mh_draft_upload_trust-test') || '{}');
  ...
  describe('trustEnabled integration', () => {
    it('preserves trustEnabled=true through save-restore cycle', () => {
      ...
      // 2. 初始化 hook 並等待自動存檔
      const { result } = renderHook(() => usePropertyDraft(formWithTrustOn, 'integration-test'));
      act(() => vi.advanceTimersByTime(1000));
    ...
    it('updates trustEnabled when form changes', () => {
      ...
      const { result, rerender } = renderHook(({ form }) => usePropertyDraft(form, 'toggle-test'), {
      ...
      // 3. 驗證新值已存檔
      const saved = JSON.parse(localStorage.getItem('mh_draft_upload_toggle-test') || '{}');
      expect(saved.trustEnabled).toBe(true);
