 ...
 */
function applyLikeToggle(comments: MockComment[], commentId: string): MockComment[] {
...
describe('留言按讚競態條件測試', () => {
  describe('applyLikeToggle 純函數', () => {
    ...
    it('反向操作應還原狀態', () => {
      const original: MockComment[] = [{ id: 'c1', isLiked: false, likesCount: 5 }];
  ...
  describe('邊界案例', () => {
    ...
    it('不存在的 commentId 應不影響狀態', () => {
      const comments: MockComment[] = [{ id: 'c1', isLiked: false, likesCount: 0 }];
    ...
    it('likesCount 不應為負數（Math.max 保護）', () => {
      const comments: MockComment[] = [{ id: 'c1', isLiked: true, likesCount: 0 }];
...
describe('模擬並發 API 請求', () => {
  ...
  it('模擬多個並發請求的執行順序', async () => {
    ...
    // 模擬 API 請求
    const mockApiCall = async (commentId: string, delay: number, shouldFail = false) => {
      // 樂觀更新
