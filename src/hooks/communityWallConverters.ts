...
export function convertApiReview(review: CommunityReview): Review {
  ...
  // 過濾 placeholder 公司名稱
  const normalizedCompany = PLACEHOLDER_COMPANY_NAMES.includes(company)
    ? ""
    : company;
...
export function convertApiQuestion(question: CommunityQuestion): Question {
  ...
  return {
    ...
    totalAnswers:
      question.totalAnswers ?? question.answers_count ?? answers.length,
  };
}
