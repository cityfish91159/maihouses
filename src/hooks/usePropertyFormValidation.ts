...
export const MAX_IMAGE_SIZE_BYTES =
  VALIDATION_RULES.images.maxSizeMB * 1024 * 1024;
...
export async function validateImageAsync(
  ...
  try {
    ...
    if (!isJpeg && !isPng && !isRiff) {
      ...
      logger.warn("[validateImageMagicBytes] Unknown file header", {
        header,
        fileName: file.name,
      });
    }
    ...
    logger.error("[validateImageMagicBytes] Magic bytes check failed", {
      error: e,
    });
 ...
 */
export async function validateImagesAsync(
  files: File[],
  chunkSize = 5,
): Promise<{
  validFiles: File[];
...
export function usePropertyFormValidation(
  ...
  const validation = useMemo(() => {
    ...
    // 價格驗證
    const priceValid =
      form.price.length >= VALIDATION_RULES.price.minLength &&
      /^\d+$/.test(form.price);
    const priceMessage = !form.price
      ? "請輸入價格"
      : !priceValid
        ? "價格必須為數字"
        : "";
    ...
    // 地址驗證
    const addressValid =
      form.address.length >= VALIDATION_RULES.address.minLength;
    const addressMessage =
      form.address.length === 0
        ? "請輸入地址"
        : addressValid
          ? ""
          : `地址至少 ${VALIDATION_RULES.address.minLength} 字`;
    ...
    const adv2Valid =
      dynamicRules.adv2Optional || adv2Length >= dynamicRules.advMin;
    ...
    const basicValid =
      titleValid && priceValid && addressValid && communityValid;
    const twoGoodOneFairValid =
      adv1Valid && adv2Valid && disValid && highlightsValid;
    const allValid =
      basicValid && twoGoodOneFairValid && imagesValid && !blockSubmit;
    ...
    return {
      ...
      advantage1: {
        ...
        contentWarning: fieldResults.find((r) => r.name === "優點1")?.result
          .issues[0]?.message,
      },
      advantage2: {
        valid: adv2Valid,
        ...
        contentWarning: fieldResults.find((r) => r.name === "優點2")?.result
          .issues[0]?.message,
      },
      disadvantage: {
        valid: disValid,
        message: disMessage,
        charCount: disLength,
        contentWarning: fieldResults.find((r) => r.name === "公道話")?.result
          .issues[0]?.message,
      },
      highlights: {
        valid: highlightsValid,
        message: highlightsMessage,
        warnings: fieldResults
          .filter((r) => r.name.startsWith("標籤"))
          .map((r) => r.result.issues[0]?.message)
