 ...
 */
export function usePropertyFormValidation(form: FormFields, imageCount: number): ValidationState {
  const validation = useMemo(() => {
    ...
    // 價格驗證
    const priceValid =
      form.price.length >= VALIDATION_RULES.price.minLength && /^\d+$/.test(form.price);
    ...
    const adv1Message = adv1Valid ? '' : `還需 ${dynamicRules.advMin - adv1Length} 字`;
    ...
    const adv2Message = adv2Valid ? '' : `還需 ${dynamicRules.advMin - adv2Length} 字`;
    ...
    const disMessage = disValid ? '' : `還需 ${dynamicRules.disMin - disLength} 字`;
