    ...
    <section className="k-span-6" id="action-panel-container">
      <div className="uag-action-panel" id="action-panel">
        <div className="ap-head">
          <span className={`uag-badge ${selectedLead.grade.toLowerCase()}`}>{selectedLead.grade}級｜{selectedLead.name}</span>
        </div>
        <div className="ap-body">
          <div className="ap-stats-group">
            <div className="ap-stat"><span>關注房源</span><b>{selectedLead.prop}</b></div>
            <div className="ap-stat"><span>意向分數</span><b style={{ color: 'var(--uag-brand)' }}>{selectedLead.intent}%</b></div>
            <div className="ap-stat"><span>瀏覽次數</span><b>{selectedLead.visit} 次</b></div>
            <div className="ap-stat"><span>購買成本</span><b>{selectedLead.price} 點</b></div>
          </div>
