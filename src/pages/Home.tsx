...
export default function Home({
  ...
  readonly config: AppConfig & RuntimeOverrides;
}) {
  // 初始化教學系統（首次訪問歡迎 + 閒置提醒）
  useTutorial();
  ...
  useEffect(() => {
    getMeta().then((r) => {
      if (r.ok && r.data) {
        if (
          r.data.maintenance ||
          cmp(r.data.backendVersion, config.minBackend) < 0
        ) {
  ...
  return (
    <>
      ...
      {banner && (
        <div className="mx-auto mt-4 max-w-container rounded-md bg-yellow-500 p-3 text-sm text-white">
          {banner}
        </div>
      )}
      <main className="relative mx-auto max-w-container space-y-6 p-4 md:space-y-8 md:p-6">
        ...
        <CommunityTeaser />
        {features.propertyGrid !== false && <PropertyGrid />}
      </main>
    </>
  );
}
