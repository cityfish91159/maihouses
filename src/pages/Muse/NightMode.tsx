  ...
  useEffect(() => {
    const loadUserData = async () => {
      ...
      if (progress) {
        ...
        // 沒有 progress 記錄：可能是首次訪問，或是被管理員刪除後回來
        const hasLocalData = localStorage.getItem('muse_streak') || localStorage.getItem('muse_last_login');
        ...
        if (!hasLocalData) {
          ...
          // 舊用戶被刪除後回來，保留 localStorage 狀態，不顯示設定畫面
          const storedStreak = parseInt(localStorage.getItem('muse_streak') || '1');
          setStreakDays(storedStreak);
          // 保留用戶的頭像和名字（如果存在）
          const savedAvatar = localStorage.getItem('muse_avatar');
          const savedName = localStorage.getItem('muse_name');
          if (savedAvatar) setMuseAvatar(savedAvatar);
          if (savedName) setMuseName(savedName);
        }

        // 🆕 建立/重建 user_progress 記錄（新用戶 or 被刪除的舊用戶）
        await supabase.from('user_progress').upsert({
          user_id: sessionId,
          sync_level: 0,
          intimacy_score: 0,
          muse_name: localStorage.getItem('muse_name') || 'MUSE',
          muse_avatar_url: localStorage.getItem('muse_avatar') || null,
          unlock_stage: 0,
