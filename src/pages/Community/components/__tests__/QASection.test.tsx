...
describe("QASection accessibility modals", () => {
  ...
  const renderComponent = (
    overrides: Partial<ComponentProps<typeof QASection>> = {},
  ) =>
    render(
      <QASection
        viewerRole="resident"
  ...
  it("focuses textarea, locks scroll, and closes on Escape", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByText("提出你的問題")).not.toBeInTheDocument(),
    );
  ...
  it("auto clears feedback after submit success", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByText(/問題已送出/)).not.toBeInTheDocument(),
    );
  });
