...
vi.mock('../Agent', () => ({
    default: ({ userId }: { userId: string }) => (
        <div data-testid="agent-page">
            ...
            <a href="/maihouses/community/test-uuid/wall" data-testid="link-wall">Community Wall</a>
            <a href="/maihouses/uag" data-testid="link-uag">UAG</a>
        </div>
    )
}));
    ...
    const builder = {
        ...
        eq: (col: string, val: string) => {
            ...
            if (col === 'id') {
                return {
                    single: () => {
                        if (val === 'real-agent') return Promise.resolve({ data: { role: 'agent' }, error: null });
                        if (val === 'real-consumer') return Promise.resolve({ data: { role: 'member' }, error: null });
