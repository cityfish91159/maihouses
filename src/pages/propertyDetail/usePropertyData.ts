...
import { propertyService, DEFAULT_PROPERTY, type PropertyData } from '../../services/propertyService';
...
function classifyLoadError(msg: string): string {
  if (msg.includes('NetworkError') || msg.includes('Failed to fetch')) return '網路連線異常，請檢查網路後重試';
...
export function usePropertyData({ id, mode, isDemoMode, mockTrustEnabled }: UsePropertyDataOptions) {
  const [property, setProperty] = useState<PropertyData>(() => ({
    ...DEFAULT_PROPERTY, publicId: id ?? DEFAULT_PROPERTY.publicId,
  }));
  const [isLoading, setIsLoading] = useState(() => Boolean(id));
  const [loadError, setLoadError] = useState<string | null>(null);
  const [reloadAttempt, setReloadAttempt] = useState(0);
  const handleRetry = useCallback(() => { setLoadError(null); setReloadAttempt((p) => p + 1); }, []);
