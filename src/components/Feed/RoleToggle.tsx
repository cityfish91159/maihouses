...
export function RoleToggle({ currentRole, onToggle, className = '' }: RoleToggleProps) {
    ...
    return (
        <button
            onClick={onToggle}
            className={`fixed bottom-24 right-4 z-modal flex items-center gap-2 rounded-full px-4 py-3 font-bold text-white shadow-lg transition-all active:scale-95 ${isAgent
