...
describe('TxBanner - MSG-3 私訊提醒橫幅', () => {
  ...
  test('1. 顯示私訊通知橫幅（有完整資訊）', () => {
    ...
    render(<TxBanner transaction={mockTransaction} messageNotification={mockNotification} />);
  ...
  test('5. 顯示交易橫幅（無私訊）', () => {
    ...
    render(<TxBanner transaction={mockTransaction} messageNotification={null} />);
  ...
  test('7. 點擊查看按鈕顯示 toast（MSG-4 未完成）', async () => {
    ...
    await waitFor(() => {
      expect(mockNotify.info).toHaveBeenCalledWith('對話功能開發中', '敬請期待');
    });
  });
