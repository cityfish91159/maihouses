...
describe("AgentSidebar", () => {
  it("renders navigation links with correct ROUTES", () => {
    ...
    expect(
      screen.getByText(STRINGS.AGENT.SIDEBAR.NAV_TITLE),
    ).toBeInTheDocument();

    const uagLink = screen.getByRole("link", {
      name: STRINGS.AGENT.SIDEBAR.LINK_UAG,
    });
    expect(uagLink).toHaveAttribute("href", ROUTES.UAG);

    const trustLink = screen.getByRole("link", {
      name: STRINGS.AGENT.SIDEBAR.LINK_TRUST,
    });
  ...
  it("renders Todo items", () => {
    ...
    expect(
      screen.getByText("Call client", { exact: false }),
    ).toBeInTheDocument();
  });
  ...
  it("renders empty state for Todos", () => {
    ...
    expect(
      screen.getByText(STRINGS.AGENT.SIDEBAR.TODO_EMPTY),
    ).toBeInTheDocument();
  });
});
