...
const PrivateWallLocked = memo(function PrivateWallLocked() {
  ...
  return (
    ...
    >
      {/* 模糊背景層 (Fake Content) */}
      <div
        className="pointer-events-none absolute inset-0 z-0 select-none bg-white/50 p-6 opacity-40 blur-md"
        aria-hidden="true"
      >
        {/* 模擬幾篇假貼文骨架 */}
        {[1, 2, 3].map((i) => (
      ...
      <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-gradient-to-b from-transparent via-white/80 to-white p-6 text-center">
        ...
        <button
          ...
          aria-label={
            isAuthenticated ? STRINGS.COMMUNITY.BTN_UNLOCK_USER : STRINGS.COMMUNITY.BTN_UNLOCK_GUEST
          }
        >
          {isAuthenticated ? STRINGS.COMMUNITY.BTN_UNLOCK_USER : STRINGS.COMMUNITY.BTN_UNLOCK_GUEST}
        </button>
