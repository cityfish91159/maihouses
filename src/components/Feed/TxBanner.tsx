 ...
 *
 * 性能優化：
 * - 使用 memo 包裹組件，避免不必要的重新渲染
 * - 使用 useMemo 快取私訊內容和交易配置
 * - 自訂比較函數，精確控制重新渲染時機
 */
export const TxBanner = memo(
  function TxBanner({ transaction, messageNotification, className = '' }: TxBannerProps) {
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="truncate text-xs text-cyan-700">
              {transactionConfig.propertyName}
              <span className="mx-1">·</span>
              {transactionConfig.stageLabel}
            </p>
          </div>
  ...
  (prevProps, nextProps) => {
    ...
    // 2a. 私訊通知存在狀態變化
    if (
      (prevMsg === null || prevMsg === undefined) !== (nextMsg === null || nextMsg === undefined)
    ) {
      return false;
    }
