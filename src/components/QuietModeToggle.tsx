  ...
  if (isActive()) {
    return (
      <button
        onClick={clearQuiet}
        title="安靜模式已啟用，點擊解除"
        className="btn-quiet on rounded-lg border border-[var(--mh-color-1749d7)] bg-[var(--mh-color-1749d7)] px-2.5 py-1.5 text-white"
      >
        安靜中（解除）
      </button>
    );
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <button
        ...
        className="btn-quiet off rounded-lg border border-[var(--mh-color-1749d7)] bg-white px-2.5 py-1.5 text-[var(--mh-color-1749d7)]"
      >
        開啟安靜模式
      </button>
      <button
        onClick={() => startQuiet({ minutes: 20, turns: 999, reason: '只想聊 20 分鐘' })}
        className="rounded-full border border-dashed border-[var(--mh-color-1749d7)] bg-[var(--mh-color-f5f8ff)] px-2 py-1 text-[var(--mh-color-1749d7)]"
      >
        只想聊 20 分鐘
      </button>
      <button
      ...
      <input
        ...
        className="w-[90px] rounded-md border border-[var(--mh-color-dddddd)] px-1.5 py-1"
      />
      <input
        id="quiet-mode-turns"
        name="quietTurns"
      ...
      <input
        ...
        className="w-[120px] rounded-md border border-[var(--mh-color-dddddd)] px-1.5 py-1"
      />
    </div>
  );
};
