    ...
    <section id="two-goods-section" className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="mt-1.5 flex items-center justify-between px-1">
            <span className={`text-[11px] font-medium ${validation.advantage1.valid ? 'text-green-600' : 'text-slate-400'}`}>
        ...
        <div>
          ...
          />
          <div className="mt-1.5 flex items-center justify-between px-1">
            <span className={`text-[11px] font-medium ${validation.advantage2.valid ? 'text-green-600' : 'text-slate-400'}`}>
        ...
        <div>
          ...
          />
          <div className="mt-1.5 flex items-center justify-between px-1">
            <span className={`text-[11px] font-medium ${validation.disadvantage.valid ? 'text-orange-600' : 'text-slate-400'}`}>
      ...
      {validation.contentCheck.hasIssues && (
        <div className={`mt-5 flex items-start gap-3 rounded-xl p-4 ${validation.contentCheck.blockSubmit ? 'border border-red-200 bg-red-50' : 'border border-yellow-200 bg-yellow-50'}`}>
          <AlertTriangle className={validation.contentCheck.blockSubmit ? 'text-red-500' : 'text-yellow-600'} size={20} />
          <div>
            <p className={`text-sm font-bold ${validation.contentCheck.blockSubmit ? 'text-red-700' : 'text-yellow-700'}`}>
              {validation.contentCheck.blockSubmit ? '內容不符合發布規範' : '內容需要注意'}
            </p>
            <ul className="mt-1.5 space-y-1 text-xs text-slate-600">
              {validation.contentCheck.warnings.map((w: string, i: number) => (
                <li key={i} className="flex items-center gap-1.5">• {w}</li>
              ))}
            </ul>
          </div>
        </div>
