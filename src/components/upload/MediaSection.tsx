          ...
          <div className="grid grid-cols-4 gap-4">
            {form.images.map((url: string, i: number) => (
              <div key={i} className="group relative aspect-square overflow-hidden rounded-xl border border-slate-200 shadow-sm">
                ...
                {i === 0 && (
                  <span className="absolute bottom-1.5 left-1.5 rounded-md bg-[#003366]/90 px-2 py-1 text-[10px] font-bold text-white backdrop-blur-sm">
                    封面
                  </span>
                )}
              </div>
            ...
            <button 
              ...
              className="flex aspect-square flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 bg-slate-50 text-slate-400 transition-all hover:border-[#00A8E8] hover:bg-blue-50 hover:text-[#00A8E8]"
            >
              <Upload size={28}/>
