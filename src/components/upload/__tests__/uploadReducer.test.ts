...
describe("uploadReducer", () => {
  ...
  describe("UPDATE_COMPRESSION_PROGRESS", () => {
    it("should update compression progress", () => {
      ...
      const newState = uploadReducer(state, {
        type: "UPDATE_COMPRESSION_PROGRESS",
        payload: 50,
      });
  ...
  describe("ADD_IMAGES", () => {
    it("should auto-set first image as cover when adding to empty state", () => {
      const images: ManagedImage[] = [
        {
          id: "img1",
          file: createMockFile("1.jpg"),
          previewUrl: "blob:1",
          isCover: false,
        },
        {
          id: "img2",
          file: createMockFile("2.jpg"),
          previewUrl: "blob:2",
          isCover: false,
        },
        {
          id: "img3",
          file: createMockFile("3.jpg"),
          previewUrl: "blob:3",
          isCover: false,
        },
        {
          id: "img4",
          file: createMockFile("4.jpg"),
          previewUrl: "blob:4",
          isCover: false,
        },
        {
          id: "img5",
          file: createMockFile("5.jpg"),
          previewUrl: "blob:5",
          isCover: false,
        },
      ];
      ...
      expect(newState.managedImages.filter((img) => img.isCover).length).toBe(
        1,
      );
      expect(newState.managedImages.slice(1).every((img) => !img.isCover)).toBe(
        true,
      );
    });
    ...
    it("should not change existing cover when adding more images", () => {
      ...
      const newImages: ManagedImage[] = [
        {
          id: "new1",
          file: createMockFile("n1.jpg"),
          previewUrl: "blob:n1",
          isCover: false,
        },
      ];

      const newState = uploadReducer(stateWithImages, {
        type: "ADD_IMAGES",
        payload: newImages,
      });
  ...
  describe("REMOVE_IMAGE", () => {
    ...
    it("should set next image as cover when removing cover image", () => {
      const images: ManagedImage[] = [
        {
          id: "cover",
          file: createMockFile("cover.jpg"),
          previewUrl: "blob:cover",
          isCover: true,
        },
        {
          id: "second",
          file: createMockFile("second.jpg"),
          previewUrl: "blob:second",
          isCover: false,
        },
        {
          id: "third",
          file: createMockFile("third.jpg"),
  ...
  describe("SET_COVER", () => {
    it("should set specified image as cover", () => {
      ...
      expect(
        newState.managedImages.find((img) => img.id === "img1")?.isCover,
      ).toBe(false);
      expect(
        newState.managedImages.find((img) => img.id === "img2")?.isCover,
      ).toBe(true);
    });
  ...
  describe("CLEAR_ERROR", () => {
    it("should clear last error", () => {
      const errorState = {
        ...initialState,
        lastError: { type: "upload" as const, message: "Error" },
      };
  ...
  describe("RESET_UPLOAD_STATE", () => {
    it("should reset to initial state", () => {
      ...
      const newState = uploadReducer(modifiedState, {
