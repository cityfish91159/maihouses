...
export const BudgetLite: React.FC = () => {
  ...
  const onCopy = () => {
    ...
    navigator.clipboard
      .writeText(text)
      .then(() => track(Events.BudgetCopy, {}));
  };

  const warmTip = (() => {
    if (burden === null) return "這個數字僅供參考，別給自己太大壓力。";
    if (burden >= 50)
      return "負擔率偏高了，先別急著決定；我們可以討論更有彈性的做法。";
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: 10, fontSize: 14, color: "#0a2246" }}>
        概算（僅供參考）：月付約 <b>{monthly.toLocaleString()}</b>{" "}
        元；貸款本金約 <b>{principal.toFixed(0)}</b> 萬。
        {burden !== null && (
          <>
