...
export const PoliteRewrite: React.FC<{ onAdopt?: (text: string) => void }> = ({
  onAdopt,
}) => {
  const [draft, setDraft] = useState("");
  const [audience, setAudience] = useState<"owner" | "agent">("agent");
  const [intent, setIntent] = useState<"view" | "detail" | "pet" | "price">(
    "detail",
  );
    ...
    >
      ...
      >
        ...
        <label>
          ...
          <select
            value={intent}
            onChange={(e) =>
              setIntent(e.target.value as "view" | "detail" | "pet" | "price")
            }
            style={{
              marginLeft: 6,
              padding: 6,
      ...
      {(v1 || v2) && (
        <div style={{ marginTop: 10, display: "grid", gap: 8 }}>
          ...
          {sent && (
            <div style={{ color: "#16a34a" }}>已準備好訊息，祝溝通順利 🤞</div>
          )}
        </div>
      )}
    </div>
