...
const DataAdapter = {
  ...
  toFeaturedSide(property) {
    ...
    return {
      ...
      details: [
        p.description,
        community.year_built ? `🏢 ${community.year_built}年完工・${community.total_units || '?'}戶` : '',
        community.owner_occupied_rate ? `👥 自住${community.owner_occupied_rate}%・${community.demographic || '住戶'}` : ''
      ].filter(Boolean),
      rating: `${p.average_rating || 4.0} 分(${p.review_count || 0} 則評價)`,
  ...
  _buildDetails(property, community) {
    ...
    if (community.year_built) {
      details.push(`🏢 ${community.year_built}年完工・${community.buildings || 1}棟・${community.total_units || '?'}戶`);
    }
