...
test.describe("Upload Flow", () => {
  ...
  test("should allow user to upload images and see compression", async ({
    page,
  }) => {
    // 找到上傳按鈕
  ...
  test("UP-3.G: should allow user to set cover image", async ({ page }) => {
    ...
    // 等待圖片加入
    await expect(page.locator(".aspect-square img")).toHaveCount(3, {
      timeout: 30000,
    });
    ...
    const secondImageStar = imageContainers
      .nth(1)
      .locator('button[title="設為封面"]');
    ...
    // 驗證只有一個封面
    await expect(
      page.locator(".aspect-square:has(img) >> text=封面"),
    ).toHaveCount(1);
  });

  test("UP-3.G: should auto-assign new cover when deleting cover image", async ({
    page,
  }) => {
    const uploadButton = page.locator('button:has-text("上傳照片")');
    await expect(uploadButton).toBeVisible({ timeout: 10000 });
    ...
    // 刪除第一張（封面）
    const deleteBtn = imageContainers
      .first()
      .locator('button[title="移除圖片"]');
  ...
  test("UP-3.G: first uploaded image should auto-become cover", async ({
    page,
  }) => {
    const uploadButton = page.locator('button:has-text("上傳照片")');
    await expect(uploadButton).toBeVisible({ timeout: 10000 });
  ...
  test("UP-3.G: only one cover should exist at any time", async ({ page }) => {
    ...
    await expect(page.locator(".aspect-square img")).toHaveCount(4, {
      timeout: 30000,
    });
