...
<<<<<<< HEAD
## 2025-12-15 Mock Data Update: Realistic Content

### 📋 工單目標
優化 Mock 資料內容，移除「測試社區」等字眼，改用真實情境內容，提升演示品質。

### 🛠️ 變更紀錄

#### 1. Mock Data (consumer.ts)
- **Content Update**: 將 ID 1002 替換為用戶指定內容：「裝潢新穎，可直接入住...」
- **Community Name**: 確保使用「惠宇上晴」等真實名稱，而非「測試社區」。

#### 2. Cache Busting (useFeedData.ts)
- **Storage Key**: `feed-mock-data-v1` -> `feed-mock-data-v2`
- 強制客戶端清除舊緩存，確保看到新資料。
