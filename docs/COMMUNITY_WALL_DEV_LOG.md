...
## 📅 2025-12-22 UP-3 圖片管理重構審計

### 📊 審計評分：75/100 ⚠️ 功能完成但無測試

| 項目 | 得分 | 扣分原因 |
|------|------|----------|
| 功能完整度 | 25/25 | UP-3.1~3.4 全部實作完成 |
| 代碼品質 | 20/25 | Reducer 架構良好，但 ManagedImage 未集中到 types/ |
| 測試覆蓋 | 10/25 | **0 個 uploadReducer 單元測試**，嚴重缺失 |
| 驗收完整度 | 20/25 | UI 功能可用，但無 E2E 驗證發布時封面位置 |

### ✅ 已完成實作

| 項目 | 檔案 | 說明 |
|------|------|------|
| UP-3.1 | `uploadReducer.ts` L22-28 | `ManagedImage { id, file, previewUrl, isCover }` |
| UP-3.2 | `uploadReducer.ts` + `UploadContext.tsx` | `useReducer` + `dispatch` 取代多個 useState |
| UP-3.3 | `uploadReducer.ts` L244-257 | `SET_COVER` action + `MediaSection.tsx` ⭐ 按鈕 |
| UP-3.4 | `uploadReducer.ts` L111-116 | `getSortedImages()` 封面排第一 |

### ❌ 審計發現缺失 (8 項)

| 編號 | 嚴重度 | 描述 |
|:---:|:---:|:---|
| UP-3.A | **P0** | 無 `uploadReducer.test.ts` 單元測試 |
| UP-3.B | P1 | `setCover` 邊界測試缺失 (不存在 id/重複設定) |
| UP-3.C | P1 | `URL.revokeObjectURL` 呼叫未驗證 (記憶體洩漏風險) |
| UP-3.D | P2 | 發布時未 assert 封面位置 |
| UP-3.E | P2 | 刪除封面後 fallback 未測試 |
| UP-3.F | P2 | 批次上傳第一張自動封面未測試 |
| UP-3.G | P3 | 無 E2E 測試驗證 UI 行為 |
| UP-3.H | P3 | `ManagedImage` 型別未移至 `types/` |
