...
| 項目 | 得分 | 說明 |
|------|------|------|
| UP-1.1 usePropertyDraft Hook | 18/25 | 基礎功能完成，但缺少 userId 整合、時間戳、版本號 |
| UP-1.2 整合至 UploadContext | 15/25 | 成功整合，但 draftFormData 每次 render 都重建 |
| UP-1.3 還原草稿 UI | 15/25 | 按鈕存在但 UX 不完整，缺少草稿預覽時間 |
| 測試覆蓋 | 17/25 | 無單元測試、無 E2E 測試 |

### 🔧 實作內容
- **新增檔案**: `src/hooks/usePropertyDraft.ts`
- **修改檔案**: 
  - `src/components/upload/UploadContext.tsx` - 整合 draft hook
  - `src/pages/PropertyUploadPage.tsx` - 新增還原草稿按鈕
- **功能**:
  - 每 1000ms 自動存入 localStorage (debounced)
  - Key 格式: `mh_draft_upload` (未依 userId 分離)
  - 發布成功後自動清除草稿
  - 頁面重整後可一鍵還原

### ❌ 發現的嚴重問題
1. **Key 格式不符規格**: TODO 寫 `mh_draft_{userId}`，實際實作為 `mh_draft_upload`
2. **無時間戳**: `getDraftPreview` 返回 `savedAt: '剛才'` 是硬編碼假資料
3. **無版本號**: 草稿結構變更時無法偵測不相容
4. **效能問題**: `draftFormData()` 在每次 render 都重新執行
5. **無測試**: 完全沒有單元測試或 E2E 測試

### 📁 修改的檔案清單
| 檔案 | 變更類型 | 說明 |
|------|----------|------|
| `src/hooks/usePropertyDraft.ts` | 新增 | 草稿自動存取 Hook |
| `src/components/upload/UploadContext.tsx` | 修改 | 整合 draft hook、clearDraft on publish |
| `src/pages/PropertyUploadPage.tsx` | 修改 | 新增還原草稿按鈕 UI |
