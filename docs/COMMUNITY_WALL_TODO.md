...
### IM-2: 591 生產級解析器 ⚠️ 78/100

**完成時間**: -
**審計評分**: 78/100 (Google 處長級審查：9 項缺失需修復)

**設計**: 帶「信心分數」的解析器，MaiMai 根據分數展現不同情緒。

| ID | 子任務 | 狀態 | 驗收標準 |
|:---|:---|:---:|:---|
| IM-2.1 | 價格解析 (售/租通用) | ⚠️ | 需正規化單位與換算 |
| IM-2.2 | 坪數解析 | ⚠️ | 需支援模糊格式 |
| IM-2.3 | 格局解析 | ⚠️ | 需支援 1.5衛/0廳/開放式 |
| IM-2.4 | 地址解析 (全台通用) | ⚠️ | 需正規化台/臺 |
| IM-2.5 | 標題擷取 | ⚠️ | 需改進評分策略 |
| IM-2.6 | 591 物件 ID 擷取 | ✅ | 匹配 `detail/123456` 等 |
| IM-2.7 | 信心分數計算 | ⚠️ | fieldsFound 計分邏輯修正 |

#### IM-2 審查 (Google 處長級) ⚠️ 78/100

| # | P | 問題 | 引導修正 |
|:---:|:---:|:---|:---|
| 2.1 | 0 | 價格未正規化：租金/售價單位混用，25,000 元/月 仍寫入 25000（萬元），缺億→萬換算與租/售標識 | 解析回傳數值+單位，統一萬為主，月租保持「萬/月」；保留原字串供 UI 顯示；支援億、萬元/坪 |
| 2.2 | 0 | fieldsFound 與分數規格不一致：格局一次 fieldsFound +=3 但規格是 5 欄各 25/25/20/20/10 | fieldsFound 僅對「價格/坪數/格局/地址/標題」各加 1；格局加分 20（套房 15）但只算一欄，ID 不加分 |
| 2.3 | 1 | 格局容錯不足：缺 1.5 衛、0 廳、開放式/無廳 | 增加半衛/零廳/開放式 regex，並為 studio 分支保持 15 分但 fieldsFound 只加一欄 |
| 2.4 | 1 | 價格/坪數缺模糊 fallback：漏「約34坪」「含車位34.2坪」「建坪28.5坪(含公設)」 | 行合併後先精準，再去除括號/字尾後寬鬆匹配；清洗「含車位/約/公設」再取數值 |
| 2.5 | 2 | 標題易誤殺：強制含房產關鍵詞，可能過濾有效標題；未做行內評分 | 對候選行計分：扣掉價格/地址/ID 行，長度 8–40；包含情緒/優點詞加分；選最高分 |
| 2.6 | 2 | 地址仍未正規化台/臺且未處理跨行 | 先合併行再跑精準→寬鬆→模糊；台/臺正規化；允許段/巷/弄/號/樓/之x 收尾 |
| 2.7 | 2 | detect591 覆蓋不足：未涵蓋元/月等低訊號 | 加入租金特徵（元/月、租金字樣）、ID 短鏈偵測；仍至少兩特徵才觸發 |
| 2.8 | 3 | 信心/欄位提示與 UX：低信心仍延遲 200ms，未回報缺失欄位 | 低信心或失敗 0ms 立即回饋並列出缺欄位；高信心可 300–500ms 展示撒花 |
| 2.9 | 3 | 測試尚未覆蓋半衛/含車位/台↔臺正規化/單位換算 | 在 parse591.test.ts 補上述案例，並驗證 fieldsFound/信心分數與規格表一致 |
