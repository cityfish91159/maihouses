...
| UP-2.J | P3 | **缺 E2E 驗收** | ✅ | 已建立 `tests/e2e/upload.spec.ts` + `playwright.config.ts`，vitest 已排除 e2e 目錄 |
...
**📝 UP-2 操作紀錄 (2025-12-22)**:
- `npm install` 補裝 heic2any，imageService.test.ts 7/7 passed
- UP-2.J 修復：playwright-chromium 版本對齊 1.57.0，新增 playwright.config.ts，vitest 排除 e2e 目錄
- UP-2.A~M 全部 ✅ 驗證完成，評分 92→100/100
