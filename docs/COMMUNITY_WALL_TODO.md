...
### 3️⃣ P0-0：community_reviews View 缺欄位（後端 500）
- **狀態**：🔴 待處理
- **嚴重度**：🔴 阻斷
- **檔案**：`supabase/migrations/20251206_fix_community_reviews_view.sql`（尚未於 Dashboard 執行）
- **描述**：現行 Supabase View 缺 `author_id` / `property_id` / `source` 欄位，API `type=reviews` 回傳 500（錯誤：column community_reviews.author_id does not exist）。
- **驗收條件**：
  - [ ] 於 Supabase Dashboard 執行 `20251206_fix_community_reviews_view.sql`
  - [ ] 驗證 `GET /api/community/wall?communityId=test-uuid&type=reviews` 回傳成功且含評價資料

### 10️⃣ P1-6：移除無意義的「社區資訊」卡片
- **狀態**：✅ 已完成
- **檔案**：`src/pages/Community/components/Sidebar.tsx`
- **描述**：Sidebar 的社區資訊卡僅顯示未知/空白，無實際價值，已移除並讓其他卡片上移。
- **驗收條件**：
  - [x] Sidebar 不再顯示「社區資訊」卡
  - [x] 頁面佈局正常、build 通過
