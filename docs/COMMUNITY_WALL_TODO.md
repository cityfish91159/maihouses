...
### ✅ P4-AUDIT-ROUND4：AI Supervisor v6.0 ELITE ENFORCER 升級 (2025-12-08)

> **目標**：強化 AI 監督系統，導入分數機制、代碼指導、10 項精英級審計檢查。

| ID | 分類 | 狀態 | 新增功能 |
|----|------|------|----------|
| v6-F1 | 💯 分數系統 | ✅ 已實作 | `cmd_score` 函數：100-150 分評分，S/A/B/C/F 等級 |
| v6-F2 | 💡 指導系統 | ✅ 已實作 | `cmd_guidance` 函數：Google L6 等級最佳實踐指南 |
| v6-F3 | 🎯 即時指導 | ✅ 已實作 | `provide_realtime_guidance`：依檔案類型 (.tsx/.ts/.css) 提供 checklist |

**v6.0 新增 10 項精英級審計檢查**：

| ID | 檢查項目 | 說明 |
|----|----------|------|
| v6-C1 | Promise 錯誤處理 | `.then()` 必須配合 `.catch()` 或 `try/catch` |
| v6-C2 | useEffect 依賴 | 空依賴數組 `[]` 警告，提醒確認是否正確 |
| v6-C3 | Tailwind 類別衝突 | `mt-X + mb-X` → `my-X`，`ml-X + mr-X` → `mx-X` |
| v6-C4 | Barrel Export | 組件目錄缺少 `index.ts` 警告 |
| v6-C5 | useCallback | 事件處理器 `handleXXX` 未用 `useCallback` 包裝警告 |
| v6-C6 | 自定義 Error | services/api 層使用普通 `Error` 警告 |
| v6-C7 | 禁止直接 DOM | `getElementById`/`querySelector`/`innerHTML` 禁用 |
| v6-C8 | Optional Chaining | `obj && obj.prop` → `obj?.prop` 建議 |
| v6-C9 | Array 直接修改 | `push`/`pop`/`splice` 在 React 狀態中警告 |
| v6-C10 | Timer 清理 | `setTimeout`/`setInterval` 必須有 `clear` 清理邏輯 |
