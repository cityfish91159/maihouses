...
- MSG-4（對話頁面）：「查看」按鈕目前顯示 toast 待實作

---

### MSG-4: 對話頁面 ⚠️ **52/100 - 需大幅改進**

**審核日期**: 2026-01-02
**審核者**: Google 首席前後端處長標準
**Commit**: `5b8c73f9` (Implement MSG-4 chat page)
...
- [x] 對話權限加入 consumer_session_id 比對
- [x] 發送訊息改為單筆插入/讀取，避免重載歷史
...
#### 1. **類型安全災難** (-15分)
**位置**: `useChat.ts` Line 84, 121, 212, 228

```typescript
// ❌ 當前做法（4 處致命缺陷）
const row = data as RawConversationRow;              // Line 84
setMessages((data as Message[]) || []);              // Line 121
const incoming = payload.new as Message;             // Line 212
const updated = payload.new as Conversation;         // Line 228
```

**問題**:
- 直接 `as` 強制轉換，零運行時驗證
- Supabase schema 改變時會靜默失敗
- JOIN 返回結構不確定，可能不是陣列

**必須改為**:
1. 定義 Zod schema 驗證所有外部數據
2. 處理驗證失敗的情況
3. 提供 fallback 默認值
...
#### 2. **安全漏洞：零權限檢查** (-10分)
**位置**: `useChat.ts` `loadConversation()` 整個函數

**問題**:
- 任何人知道 `conversationId` 就能看對話！
- 沒有檢查 `user.id` 是否是 `agent_id` 或 `consumer_profile_id`
- 可能洩露其他用戶的私密訊息
...
// 查詢後立即驗證權限
const isParticipant =
  (isAgent && row.agent_id === user.id) ||
  (!isAgent && (row.consumer_profile_id === user.id || row.consumer_session_id === sessionId));
...
#### 3. **性能災難：發送訊息重查全部** (-10分)
**位置**: `useChat.ts` Line 154

```typescript
// ❌ 當前做法
await loadMessages();  // 發送後重新查詢所有訊息！
```

**問題**:
- 每次發送都查詢整個對話歷史
- 100 條訊息就重新查 100 條
- Realtime 已經會推送新訊息，完全多餘
...
#### 4. **Realtime 訂閱低效排序** (-8分)
**位置**: `useChat.ts` Line 217
...
// 或加入防禦性檢查
const lastTimestamp = prev[prev.length - 1]?.created_at;
if (lastTimestamp && incoming.created_at < lastTimestamp) {
  // 只在亂序時排序
...
```typescript
  ...
  logger.error('MessageInput.send.failed', { err, content: trimmed });
  // 保留輸入內容讓用戶重試
}
```
...
├── useChat.test.ts           # Hook 邏輯測試
├── MessageInput.test.tsx     # 輸入組件測試
├── MessageList.test.tsx      # 列表渲染測試
└── ChatPage.integration.test.tsx  # 整合測試
```
...
#### 7. **JOIN 數據結構混亂**
**位置**: `useChat.ts` Line 73-75, 87

```typescript
// Supabase JOIN 返回的是陣列？
agent_profile:profiles!agent_id(name, email),  // 為何是陣列？
// ...
...
**建議**:
檢查 Supabase 查詢語法，確保返回單一物件，或明確定義為陣列類型。

---

#### 8. **無訊息長度限制**
**位置**: `MessageInput.tsx` 缺少 `maxLength`
...
#### 9. **可訪問性不足**
**位置**: `MessageInput.tsx` Line 26

**問題**:
- `<textarea>` 缺少 `aria-label`
- 盲人用戶不知道這是訊息輸入框
...
#### 10. **重複代碼**
**位置**: `index.tsx` Line 21-56

**問題**:
三個 if 塊都有相同的 Header + 容器結構
...
1. ✅ TypeScript 編譯通過（+2分）
2. ✅ 無 console.log（+2分）
3. ✅ 使用 logger（+2分）
4. ✅ Realtime 訂閱正確清理（+2分）
...
**P0 - 立即修復（安全與正確性）**:
- [ ] 1. 新增 Zod schema 驗證所有 Supabase 數據
- [ ] 2. 實作權限檢查（驗證用戶是對話參與者）
- [ ] 3. 移除 `loadMessages()` 改為樂觀更新
- [ ] 4. 修復 Realtime 排序邏輯
- [ ] 5. MessageInput 加入錯誤提示 toast

**P1 - 品質提升**:
- [ ] 6. 新增完整測試套件（最少 15 個測試）
- [ ] 7. 訊息長度限制 + 字數統計
- [ ] 8. 可訪問性 aria-label
- [ ] 9. 圖片加載失敗 fallback
- [ ] 10. 抽取重複的 ErrorLayout 組件
...
完成 P0: **75/100** ✅ 及格
完成 P0 + P1: **90/100** ⭐ 優秀
完成 P0 + P1 + P2: **100/100** 🏆 完美

**當前結論**:
雖然基本功能可運作，但存在**嚴重的安全漏洞**和**性能問題**，**不建議上線**。必須先完成 P0 所有修復才能部署到生產環境。
