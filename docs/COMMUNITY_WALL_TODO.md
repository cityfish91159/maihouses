...
### UP-3: 圖片管理重構 ✅ 90/100 (2025-12-22)

**📊 審計評分：90/100** ✅ 核心邏輯修正，E2E 建設中
...
|:---:|:---:|:---|:---|:---|
| UP-3.A | **P0** | **無 uploadReducer 單元測試** | ✅ | `uploadReducer.test.ts` 28 個測試案例 (已修正 SET_COVER 邊界行為) |
| UP-3.B | **P1** | **setCover 未測試邊界** | ✅ | 已修正：不存在 id 時不清除既有封面 |
| UP-3.C | **P1** | **previewUrl 記憶體洩漏風險** | ✅ | mock `revokeObjectURL` 驗證呼叫次數 |
| UP-3.D | **P2** | **發布時未驗證封面位置** | ✅ | 已修正 assertion 邏輯：無封面時會正確報錯 |
| UP-3.E | **P2** | **刪除封面後的 fallback 未測** | ✅ | 刪除封面→第一張成封面 已測試 |
| UP-3.F | **P2** | **批次上傳第一張自動封面未測** | ✅ | 5張只有第一張isCover:true 已測試 |
| UP-3.G | **P3** | **無 E2E 測試驗證 UI 行為** | ⚠️ | **建設中**：環境修復 (env vars)，但測試邏輯需依照實際 UI 調整 (1/5 通過) |
| UP-3.H | **P3** | **ManagedImage 型別未 export 到 types/** | ✅ | 已修正：`src/types/upload.ts` 改為 re-export，消除重複定義 |

**📝 UP-3 操作紀錄 (2025-12-22)**:
- 功能實作完成：ManagedImage / uploadReducer / setCover / getSortedImages
- 審計缺失 A/B/C/E/F/H 修正，G (E2E) 部分完成
- Tests: 29 passed (28 unit + 1 E2E) | Build: ✅
