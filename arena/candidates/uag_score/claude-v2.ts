...
const lvl = (s: number): "S" | "A" | "B" | "C" | "F" =>
  s >= 90 ? "S" : s >= 75 ? "A" : s >= 60 ? "B" : s >= 40 ? "C" : "F";
...
export function uagScore(input: unknown): O {
  ...
  const quality = Math.min(
    25,
    Math.floor(Math.max(0, Math.min(5, n(d.avgRating, 0))) * 5),
  );
  const responsiveness = Math.max(
    0,
    Math.floor(25 - n(d.responseTimeHours, 999) * 2),
  );
  const history = Math.min(
    20,
    Math.floor(
      Math.max(0, n(d.reviewCount, 0)) +
        Math.max(0, n(d.updateFrequency, 0)) * 2,
    ),
  );
