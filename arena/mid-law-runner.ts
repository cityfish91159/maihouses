...
export function preWriteCheck(taskName: string): MidLawCheckResult {
  ...
  if (!fs.existsSync(tracePath)) {
    const initialTrace: TaskTrace = { taskName, startTime: Date.now(), attempts: 0, failed: 0, versions: [] };
    fs.mkdirSync(path.dirname(tracePath), { recursive: true });
    fs.writeFileSync(tracePath, JSON.stringify(initialTrace, null, 2));
    recommendations.push(`已建立 traces/${taskName}.json`);
  }

  const existingVersions = fs.existsSync(candidatesDir) ? fs.readdirSync(candidatesDir).filter(f => f.endsWith(".ts")).length : 0;
  if (existingVersions < FORCE_EXPLORE.minVersions) {
    recommendations.push(`📝 需要至少 ${FORCE_EXPLORE.minVersions} 個版本，目前 ${existingVersions} 個`);
  }

  return { passed: violations.length === 0, violations, totalPenalty: 0, recommendations };
}
...
export function duringWriteCheck(taskName: string): MidLawCheckResult {
  ...
  const candidates = fs.existsSync(candidatesDir) ? fs.readdirSync(candidatesDir).filter(f => f.endsWith(".ts")) : [];
  ...
  if (candidates.length < FORCE_EXPLORE.minVersions) {
    violations.push({ law: 7, severity: "warning", message: `只有 ${candidates.length} 個版本`, penalty: FORCE_EXPLORE.singleVersionPenalty });
  }

  return { passed: violations.filter(v => v.severity === "fatal").length === 0, violations, totalPenalty: violations.reduce((s, v) => s + v.penalty, 0), recommendations };
}
...
export function finishCheck(taskName: string): MidLawCheckResult {
  ...
  if (!fs.existsSync(tracePath)) {
    return { passed: false, violations: [{ law: 0, severity: "fatal", message: "沒有 trace", penalty: -100 }], totalPenalty: -100, recommendations };
  }

  const trace: TaskTrace = JSON.parse(fs.readFileSync(tracePath, "utf-8"));

  if (trace.failed < ANTI_REGRESSION.minFailedAttempts) {
    violations.push({ law: 1, severity: "warning", message: `只有 ${trace.failed} 次失敗`, penalty: ANTI_REGRESSION.onePassPenalty });
    recommendations.push("⚠️ 一次就成功 = 高風險投機標記");
  }

  if (trace.versions.length < FORCE_EXPLORE.minVersions) {
    violations.push({ law: 7, severity: "error", message: `只有 ${trace.versions.length} 個版本`, penalty: FORCE_EXPLORE.singleVersionPenalty });
  }

  return { passed: violations.filter(v => v.severity === "fatal" || v.severity === "error").length === 0, violations, totalPenalty: violations.reduce((s, v) => s + v.penalty, 0), recommendations };
}
...
export function recordAttempt(taskName: string, versionName: string, success: boolean, metrics: Partial<VersionRecord>): void {
  const tracePath = path.join(__dirname, "traces", `${taskName}.json`);
  let trace: TaskTrace = fs.existsSync(tracePath) ? JSON.parse(fs.readFileSync(tracePath, "utf-8")) : { taskName, startTime: Date.now(), attempts: 0, failed: 0, versions: [] };
  ...
  if (existing) {
    ...
    trace.versions.push({ name: versionName, createdAt: Date.now(), lines: metrics.lines || 0, avgRuntimeMs: metrics.avgRuntimeMs, testsPassed: success, eliminated: !success, eliminationReason: metrics.eliminationReason, writeDurationMs: metrics.writeDurationMs || 0, testRuns: 1, fixRounds: 0 });
  }
...
switch (command) {
  case "pre-write":
    if (!taskName) { log.error("需要 task name"); process.exit(1); }
