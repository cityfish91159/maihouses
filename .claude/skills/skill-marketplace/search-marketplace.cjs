...
async function searchMarketplace(query, options = {}) {
  ...
  if (apiResults.skills && apiResults.skills.length > 0) {
    console.log(
      `✅ 從 skillsmp.com API 找到 ${apiResults.skills.length} 個結果`,
    );
  ...
  if (results.length === 0) {
    ...
    console.log("\n📥 手動安裝:");
    console.log(
      "   node .claude/skills/skill-marketplace/install-skill.cjs <github-raw-url>",
    );
    return [];
  }
...
if (require.main === module) {
  ...
  if (!query) {
    console.log(
      'Usage: node search-marketplace.cjs "<search query>" [category] [limit]',
    );
    console.log("");
