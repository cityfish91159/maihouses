  ...
  try {
    switch (req.method) {
            ...
            getSupabase()
              ...
              .then(({ data: post, error: postError }) => {
                ...
                if (post.community_id !== communityId)
                  return { type: 'post', valid: false, error: 'communityId 與貼文所屬社區不符' };
              ...
              getSupabase()
                ...
                .then(({ data: parentComment, error: parentError }) => {
                  if (parentError || !parentComment)
                    return { type: 'parent', valid: false, error: '父留言不存在' };
                  if (parentComment.post_id !== postId)
                    return { type: 'parent', valid: false, error: 'parentId 與 postId 不符' };
      ...
      case 'PUT': {
        ...
        if (error) {
          if (error.code === 'PGRST116') {
            // No rows returned
            return res.status(403).json({ success: false, error: '無權編輯此留言或留言不存在' });
          }
          throw error;
        }
        return res.status(200).json({ success: true, data });
      ...
      case 'DELETE': {
        ...
        if (error) throw error;
        if (count === 0) {
          return res.status(403).json({ success: false, error: '無權刪除此留言或留言不存在' });
        }
