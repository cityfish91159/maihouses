...
vi.mock('../../lib/apiResponse', async () => {
  const actual =
    await vi.importActual<typeof import('../../lib/apiResponse')>('../../lib/apiResponse');
  return {
    ...actual,
...
describe('Data transformation helpers', () => {
  ...
  it('transformReviewRecord 應使用 property_id 查找房源資訊', () => {
    ...
    const propertyMap = new Map([[PROPERTY_ID, buildProperty({ title: '測試房源' })]]);
  ...
  it('transformReviewRecord 應處理同時有 agent 和 property 資訊', () => {
    ...
    const propertyMap = new Map([[PROPERTY_ID, buildProperty({ title: '房源A' })]]);
    const agentMap = new Map([[AGENT_ID, buildAgent({ name: '房仲A' })]]);
