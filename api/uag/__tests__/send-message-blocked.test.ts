...
describe('測3：封鎖 OA 處理邏輯', () => {
  ...
  interface SendMessageResponse {
    ...
    lineStatus: 'sent' | 'pending' | 'no_line' | 'unreachable' | 'error' | 'skipped';
    error?: string;
  }
   ...
   */
  function updateNotificationStatus(purchaseId: string, status: string): void {
    // 模擬資料庫更新
  ...
  describe('Response 格式', () => {
    ...
    it('unreachable 應該是有效的 lineStatus 值', () => {
      ...
      const unreachableStatus: SendMessageResponse['lineStatus'] = 'unreachable';
  ...
  describe('Toast 訊息邏輯', () => {
    it("lineStatus: 'unreachable' 應對應 Toast 顯示「LINE 無法送達」", () => {
      ...
      // 模擬前端 Toast 邏輯
      function getToastMessage(lineStatus: SendMessageResponse['lineStatus']): string {
        switch (lineStatus) {
          case 'sent':
