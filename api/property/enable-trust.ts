 ...
 */
async function verifyAgentAuth(req: VercelRequest): Promise<AuthResult | AuthError> {
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  if (!authResult.success) {
    res.status(authResult.status).json(errorResponse(authResult.code, authResult.message));
    return;
  }
  ...
  try {
    ...
    if (fetchError || !property) {
      ...
      res.status(404).json(errorResponse(BE2_ERROR_CODES.PROPERTY_NOT_FOUND, '找不到此物件'));
      return;
    }
    ...
    if (!propertyResult.success) {
      ...
      res.status(500).json(errorResponse(API_ERROR_CODES.INTERNAL_ERROR, '物件資料格式異常'));
      return;
    }
    ...
    if (validProperty.agent_id !== agentId) {
      ...
      res.status(403).json(errorResponse(BE2_ERROR_CODES.NOT_OWNER, '您不是此物件的擁有者'));
      return;
    }
    ...
    if (validProperty.trust_enabled) {
      ...
      res
        .status(400)
        .json(errorResponse(BE2_ERROR_CODES.ALREADY_ENABLED, '安心服務已開啟，無法重複操作'));
      return;
    }
    ...
    if (updateError) {
      ...
      res.status(500).json(errorResponse(API_ERROR_CODES.INTERNAL_ERROR, '更新失敗'));
      return;
    }
