...
function adaptRealPropertyForUI(
  row: RealPropertyRow,
  reviews: ReviewData[],
): PropertyForUI {
  // 1. 圖片處理 (取第一張 + 強制裁切)
  ...
  */
  ...
  if (displayReviews.length < 2) {
    ...
    displayReviews.push(
      ...defaultSets[seedIndex].slice(0, 2 - displayReviews.length),
    );
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (realData && realData.length > 0) {
      // 收集不重複的 community_id
      const communityIds = [
        ...new Set(realData.map((p) => p.community_id).filter((id) => id)),
      ];
