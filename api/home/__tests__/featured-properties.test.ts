...
const buildPropertyRow = (overrides?: Partial<RealPropertyRow>): RealPropertyRow => ({
...
describe('featured-properties helpers', () => {
  ...
  describe('adaptRealPropertyForUI', () => {
    ...
    it('多樣化補位評價 (無評價時自動填充)', () => {
      const row = buildPropertyRow({ id: 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeee00' }); // 末碼 0
      const result = adaptRealPropertyForUI(row, []);
    ...
    it('UUID 末碼決定不同預設評價組', () => {
      // 末碼 0 -> seedIndex 0
      const row0 = buildPropertyRow({ id: 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeee00' });
      const result0 = adaptRealPropertyForUI(row0, []);
      
      // 末碼 5 -> seedIndex 5 % 3 = 2
      const row5 = buildPropertyRow({ id: 'aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeee05' });
  ...
  describe('SERVER_SEEDS', () => {
    ...
    it('每筆 seed 有必要欄位', () => {
      const requiredFields = ['id', 'image', 'badge', 'title', 'tags', 'price', 'location', 'reviews', 'source'];
