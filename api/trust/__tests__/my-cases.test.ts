...
function mockModules(options: MockOptions = {}) {
  ...
  const queryCasesByIdentityMock = vi
    .fn()
    .mockImplementation((params: { userId?: string; lineUserId?: string }) => {
      if (params.userId) {
  ...
  vi.doMock('../services/case-query', () => ({
    ...
    generateTrustRoomUrl: vi
      .fn()
      .mockImplementation(
        (id: string) => `https://maihouses.vercel.app/maihouses/#/trust-room/${id}`
      ),
  }));
...
describe('BE-6 trust/my-cases handler', () => {
  ...
  describe('system-key 認證模式', () => {
    ...
    it('業務邏輯層 LINE ID 錯誤回傳 400', async () => {
      mockModules({
        lineQueryResult: { success: false, error: 'LINE ID error', code: 'INVALID_LINE_ID' },
      });
      const { default: handler } = await import('../my-cases');
      const req = {
        method: 'GET',
