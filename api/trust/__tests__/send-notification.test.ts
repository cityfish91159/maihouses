...
describe('BE-8 | sendNotification - 核心函數', () => {
  ...
  describe('Push 發送', () => {
    ...
    it('410 Gone 時應刪除過期訂閱', async () => {
      ...
      expect(localMockDeleteIn).toHaveBeenCalledWith('endpoint', [
  ...
  describe('LINE 發送', () => {
    it('LINE 成功應返回 channel: line', async () => {
      ...
      vi.doMock('../notify', () => ({
        getNotifyTarget: () => Promise.resolve({ type: 'line', lineId: VALID_LINE_ID }),
      }));
    ...
    it('LINE 可接受大寫 LINE ID', async () => {
      ...
      vi.doMock('../notify', () => ({
        getNotifyTarget: () => Promise.resolve({ type: 'line', lineId: VALID_LINE_ID_UPPER }),
      }));
  ...
  describe('Push → LINE 降級機制', () => {
    it('Push 失敗 → 重試失敗 → LINE 降級成功', async () => {
      ...
      const { sendNotification, setPushSender, resetPushSender } =
        await import('../send-notification');
    ...
    it('Push 失敗 → 重試失敗 → LINE 降級也失敗', async () => {
      ...
      const localMockPushMessage = vi.fn().mockRejectedValue(new Error('LINE rate limited'));
      const localMockSingle = vi.fn().mockResolvedValue({
        data: {
          buyer_user_id: VALID_USER_ID,
          buyer_line_id: VALID_LINE_ID,
...
describe('BE-8 | 便利函數', () => {
  ...
  describe('sendStepUpdateNotification', () => {
    it('應正確生成步驟更新訊息並發送', async () => {
      ...
      const { sendStepUpdateNotification } = await import('../send-notification');

      const result = await sendStepUpdateNotification(VALID_CASE_ID, 2, 3, '信義區三房');
  ...
  describe('sendCaseClosedNotification', () => {
    it('closed_sold_to_other 應生成正確訊息', async () => {
      ...
      const { sendCaseClosedNotification } = await import('../send-notification');
    ...
    it('closed_property_unlisted 應生成正確訊息', async () => {
      ...
      const result = await sendCaseClosedNotification(VALID_CASE_ID, 'closed_property_unlisted');
    ...
    it('closed_inactive 應生成正確訊息', async () => {
      ...
      const result = await sendCaseClosedNotification(VALID_CASE_ID, 'closed_inactive');
  ...
  describe('sendCaseWakeNotification', () => {
    it('應正確生成喚醒訊息', async () => {
      ...
      const result = await sendCaseWakeNotification(VALID_CASE_ID, '中山區套房');
...
describe('BE-8 | 類型安全驗證', () => {
  ...
  it('SendResult 類型結構正確', () => {
    type SendChannel = 'push' | 'line' | 'fallback_line' | 'none' | 'skipped';
