...
import { successResponse, errorResponse, API_ERROR_CODES, API_WARNING_CODES } from '../apiResponse';
...
describe('apiResponse', () => {
  describe('successResponse', () => {
    ...
    it('應正確處理警告訊息', () => {
      const data = { items: [] };
      const warnings = [{ code: 'PARTIAL_FAILURE', message: '部分資料載入失敗' }];
  ...
  describe('API_ERROR_CODES', () => {
    ...
    it('應包含業務邏輯錯誤碼', () => {
      ...
      expect(API_ERROR_CODES.FORBIDDEN_PRIVATE_POSTS).toBe('FORBIDDEN_PRIVATE_POSTS');
      expect(API_ERROR_CODES.REVIEW_FETCH_FAILED).toBe('REVIEW_FETCH_FAILED');
      expect(API_ERROR_CODES.PROPERTY_FETCH_FAILED).toBe('PROPERTY_FETCH_FAILED');
  ...
  describe('API_WARNING_CODES', () => {
    it('應包含所有警告碼', () => {
      ...
      expect(API_WARNING_CODES.REVIEWS_FETCH_FAILED).toBe('REVIEWS_FETCH_FAILED');
      expect(API_WARNING_CODES.AGENT_DATA_UNAVAILABLE).toBe('AGENT_DATA_UNAVAILABLE');
      expect(API_WARNING_CODES.COMMUNITY_DATA_INCOMPLETE).toBe('COMMUNITY_DATA_INCOMPLETE');
    });
  });
  ...
  describe('實際使用場景', () => {
    ...
    it('應支援開發環境調試', () => {
      ...
      const result = errorResponse('VALIDATION_ERROR', '輸入格式錯誤', validationErrors);
