...
async function fetchSkillsMP(endpoint, params = {}) {
  ...
  if (!response.ok) {
    throw new Error(
      `SkillsMP API error: ${response.status} ${response.statusText}`,
    );
  }
    ...
    tools: [
      {
        name: "search_skills",
        description:
          "Search for agent skills by keyword. Returns skills from the SkillsMP marketplace that match the query. Use this to find tools and capabilities for coding tasks.",
        inputSchema: {
          type: "object",
          properties: {
            query: {
      ...
      },
      {
        ...
        description:
          "AI-powered semantic search for agent skills. Uses AI to understand the intent and find the most relevant skills. Better for natural language queries.",
        inputSchema: {
          type: "object",
          properties: {
            query: {
      ...
      },
      {
        ...
        description:
          "Get skill recommendations for a specific task. Analyzes the task and suggests relevant skills that could help accomplish it.",
        inputSchema: {
          type: "object",
          properties: {
            task: {
            ...
            context: {
              type: "string",
              description:
                "Additional context about the project or environment",
            },
          },
          required: ["task"],
        },
  ...
  try {
    switch (name) {
      ...
      case "recommend_skills_for_task": {
        ...
        const enhancedQuery = context ? `${task}. Context: ${context}` : task;
