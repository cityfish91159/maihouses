...
function checkSkillsMPAvailable() {
...
// 前置任務檢查
function preTaskCheck(taskDescription) {
...
// 根據任務描述建議關鍵字
function suggestKeywords(taskDescription) {
  const keywords = [];
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      log('\n使用方式:', 'cyan');
      log('  node scripts/ai-workflow-hooks.js pre-task "任務描述"', 'reset');
      log('  node scripts/ai-workflow-hooks.js post-task', 'reset');
      log('  node scripts/ai-workflow-hooks.js check-code <file1> <file2>', 'reset');
      log('  node scripts/ai-workflow-hooks.js check-skillsmp', 'reset');
      break;
  }
}
