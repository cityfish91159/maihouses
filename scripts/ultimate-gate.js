...
function runStep(stepName, command, args, opts = {}) {
  ...
  if (result.status !== 0) {
    ...
    if (opts.fatal !== false) {
      console.log(
        `${c.red}👉 錯誤發生在指令: ${command} ${args.join(" ")}${c.reset}`,
      );
      process.exit(1);
    }
  ...
  if (opts.hideOutput)
    console.log(
      `${c.green}✅ 通過${c.reset} ${c.gray}(${duration}s)${c.reset}`,
    );
  else
    console.log(
      `${c.green}✅ ${stepName} 通過${c.reset} ${c.gray}(${duration}s)${c.reset}`,
    );
...
function deepScan() {
  ...
  function walkDir(dir) {
    if (
      dir.match(/node_modules|\.next|\.git|dist|build|coverage|muse|god-muse/i)
    )
      return;
    ...
    for (const file of files) {
      ...
      if (file.match(/muse|god-muse|\.d\.ts|\.map|\.test\.|\.spec\./i))
        continue;
...
function checkBundleBudgets() {
  ...
  const buildDir = fs.existsSync("dist")
    ? "dist"
    : fs.existsSync(".next")
      ? ".next"
      : null;
  ...
  try {
    ...
    if (failed) {
      console.log(
        `${c.yellow}⚠️  建議使用 Dynamic Import (React.lazy) 拆分代碼。${c.reset}`,
      );
