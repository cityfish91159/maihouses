...
const BASE_URL =
  process.env.P5_URL || "https://maihouses.vercel.app/maihouses/property.html";
...
async function runHappyPath(page: Page) {
  ...
  const { renderLog, telemetry, listingCount, mainHtml, tagsRendered } =
    await page.evaluate(() => {
      ...
      const listings = document.querySelectorAll(
        "#listing-grid-container article",
      ).length;
      const mainInner =
        (document.getElementById("featured-main-container") || {}).innerHTML ||
        "";

      // KC-5.3: 驗證 tags 有被渲染
      const tagChips = document.querySelectorAll(
        '.tag-chip, [data-testid="tag-chip"], .bg-amber-50',
      );
  ...
  assert(
    Array.isArray(renderLog) && renderLog.length > 0,
    "render log missing",
  );
  assert(
...
async function runFallbackTest(page: Page) {
  ...
  await page.goto(`${BASE_URL}?fallback=${Date.now()}`, {
    waitUntil: "networkidle",
  });
  await page.waitForTimeout(500);

  const { telemetry, renderLog, listingCount, seedTagsRendered } =
    await page.evaluate(() => {
      const win = window as unknown as WindowWithApi;
      ...
      return {
        telemetry: win.__phase4Telemetry || {
          events: [],
          lcp: null,
          fcp: null,
        },
        renderLog: win.__renderVersionLog || [],
        listingCount: document.querySelectorAll(
          "#listing-grid-container article",
        ).length,
  ...
  const lastEvent =
    Array.isArray(telemetry.events) && telemetry.events.length > 0
  ...
  assert(
    Array.isArray(renderLog) && renderLog.length > 0,
    "render log missing under fallback",
  );
...
async function runRaceGuardTest(page: Page, seed: any) {
  ...
  await page.goto(`${BASE_URL}?race=${Date.now()}`, {
    waitUntil: "networkidle",
  });
  ...
  assert(
    isObject(results[1]) && "featured" in results[1],
    "second call should succeed with data",
  );
